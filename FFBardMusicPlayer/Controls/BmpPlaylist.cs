...
namespace FFBardMusicPlayer.Controls {
	public partial class BmpPlaylist : UserControl {
		...
		public BmpPlaylist() {
			...
			AutoPlayToggle.Checked = BmpPigeonhole.Instance.PlaylistAutoPlay;
			...
			playlistDelay = BmpPigeonhole.Instance.PlaylistDelay;
		...
		private void Playlist_AutoPlay_CheckedChanged(object sender, EventArgs e)
		{
			BmpPigeonhole.Instance.PlaylistAutoPlay = AutoPlayToggle.Checked;
		}
		...
		private void Playlist_Load_Click(object sender, EventArgs e)
		{
			...
			openFileDialog1.Filter = "pls files (*.pls)|*.pls|All files (*.*)|*.*";
			...
			if (openFileDialog1.ShowDialog() == DialogResult.OK)
			{
				...
				using (StreamReader sr = new StreamReader(fileStream))
				{
					while (!sr.EndOfStream)
					{
						...
						int track = Convert.ToInt32(data.Split('{')[1].Split('}')[0]);
						AddPlaylistEntry(filename, track);
					}
		...
		private void Playlist_Save_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Right)
			{
				if (PlaylistView.RowCount != 0)
				{
					...
					saveFileDialog1.Filter = "pls files (*.pls)|*.pls|All files (*.*)|*.*";
					saveFileDialog1.FilterIndex = 2;
					saveFileDialog1.RestoreDirectory = true;
					if (saveFileDialog1.ShowDialog() == DialogResult.OK)
					{
						for (int index = 0; index < PlaylistView.RowCount; index++)
						{
							DataGridViewRow row = PlaylistView.Rows[index];
							if (row != null && row.DataBoundItem is BmpMidiEntry entry)
							{
								for (int tnum = 1; tnum <= 8; tnum++)
								{
									using (StreamWriter sw = new StreamWriter(saveFileDialog1.FileName + tnum.ToString()+".pls", true, System.Text.Encoding.Unicode))
									{
										string filename = entry.FilePath.FilePath;
										int track = entry.Track.Track;
										row.Selected = true;
										sw.Write(filename);
										sw.Write("{");
										sw.Write(tnum);
		...
		private void Playlist_Save_Click(object sender, EventArgs e)
       {	
			if (PlaylistView.RowCount != 0)
			{
				SaveFileDialog saveFileDialog1 = new SaveFileDialog();
